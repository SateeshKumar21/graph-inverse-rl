...
import sys
import os
from graphirl.bbox_util import *
...
class VerticalFlip(object):
    ...
    def __call__(self, img, bboxes):
            ...
            bboxes = np.column_stack([bboxes, pairwise_distances(centres)])
...
class Shift(object):
    ...
    def __init__(self):
       self.offset = (np.random.rand(1)*0.6 - 0.4) 

    def __call__(self, img, bboxes):
            bboxes = bboxes[:, :4]
            bboxes = (self.offset*img.shape[0]) + bboxes
            centres_y, centres_x = (bboxes[:, 3] + bboxes[:, 1])/2, (bboxes[:, 2] + bboxes[:, 0])/2 
            centres = np.column_stack([centres_x, centres_y])            
            bboxes = np.column_stack([bboxes, pairwise_distances(centres)])
            return img, bboxes
