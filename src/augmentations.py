...
def random_overlay(x, dataset='places365_standard', use_val=False):
	...
	if dataset == 'places365_standard':
		if places_dataloader is None:
			_load_places(batch_size=x.size(0), image_size=x.size(-1), use_val=use_val)
...
def random_affine(x, angle=8, scale=0.08, shear=8):
	assert isinstance(x, torch.Tensor) and x.is_cuda, \
		'input must be CUDA tensor'
	return kornia.augmentation.RandomAffine(
		p=1.,
		degrees=angle,
		scale=(1-scale, 1+scale, 1-scale, 1+scale),
		shear=shear,
		padding_mode='border'
	)(x/255.)*255.
...
def random_noise(x):
	assert isinstance(x, torch.Tensor) and x.is_cuda, \
		'input must be CUDA tensor'
	alpha = 0.4
	noise = torch.empty(x.size(0), 3, x.size(2), x.size(3), device='cuda:0') \
		.uniform_(0, 1) \
		.repeat(1, x.size(1)//3, 1, 1)
	return ((1-alpha)*(x/255.) + (alpha)*noise)*255.
...
def random_crop(x, size=84, w1=None, h1=None, return_w1_h1=False):
	...
	print("Cropped tensor shape ", cropped.shape)
