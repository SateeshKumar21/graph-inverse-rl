...
class BaseEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        self.distance_threshold_2 = distance_threshold
    ...
    def _get_obs(self):
        ...
        elif self.observation_type in {'image', 'state+image', 'statefull+image'} and not self.render_for_human:
        ...
        if self.observation_type=='statefull+image':
            state_ = self._get_state_obs()
        elif self.observation_type=='state+image':
    ...
    def _is_success_2(self, acheived_goal, desired_goal):
        
        d = self.goal_distance(acheived_goal, desired_goal, self.use_xyz)
        
        return (d < self.distance_threshold_2).astype(np.float32)

    def _is_success_3(self, acheived_goal, desired_goal):
        
        d = self.goal_distance(acheived_goal, desired_goal, self.use_xyz)
        
        return (d < self.distance_threshold_3).astype(np.float32)
