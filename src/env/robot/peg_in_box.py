...
class PegBoxEnv(BaseEnv, utils.EzPickle):
	def __init__(self, xml_path, cameras, n_substeps=20, observation_type='image', reward_type='dense', image_size=84, use_xyz=False, render=False):
		...
		self.distance_threshold_2 = 0.05
		self.distance_threshold_3 = 0.15
	...
	def _is_success_2(self, desired_goal):

		achieved_goal = self.sim.data.get_site_xpos('object0').copy()

		return BaseEnv._is_success_2(self, achieved_goal, desired_goal)

	def _is_success_3(self, desired_goal):

		achieved_goal = self.sim.data.get_site_xpos('object0').copy()

		return BaseEnv._is_success_3(self, achieved_goal, desired_goal)


	def _get_distance(self, desired_goal):
	
		achieved_goal = self.sim.data.get_site_xpos('object0')
		
		return BaseEnv.goal_distance(self, achieved_goal, desired_goal, self.use_xyz)

	def step(self, action):

		obs, env_reward, done, info = super(PegBoxEnv, self).step(action)

		info['success_rate_05'] = self._is_success_2(self.goal)
		info['success_rate_15'] = self._is_success_3(self.goal)
		info['distance'] = self._get_distance(self.goal)
