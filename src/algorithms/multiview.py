...
class MultiView(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		shared_cnn_1 = m.SharedCNN(obs_shape, args.num_shared_layers, args.num_filters, args.mean_zero).cuda() # Third Person Image
		shared_cnn_2 = m.SharedCNN(obs_shape, args.num_shared_layers, args.num_filters, args.mean_zero).cuda() # First Person Image

		integrator = m.Integrator(shared_cnn_1.out_shape, shared_cnn_2.out_shape, args.num_filters).cuda() # Change channel dimensions of concatenated features

		assert shared_cnn_1.out_shape==shared_cnn_2.out_shape
		head_cnn = m.HeadCNN(shared_cnn_1.out_shape, args.num_head_layers, args.num_filters).cuda() # Pass concatenated features into the head

		actor_encoder = m.MultiViewEncoder(
			shared_cnn_1,
			shared_cnn_2,
			integrator,
			head_cnn,
		...
		critic_encoder = m.MultiViewEncoder(
			shared_cnn_1,
			shared_cnn_2,
			integrator,
			head_cnn,
		...
		self.actor = m.Actor(actor_encoder, action_shape, args.hidden_dim, args.actor_log_std_min, args.actor_log_std_max, multiview=True).cuda()
		self.critic = m.Critic(critic_encoder, action_shape, args.hidden_dim, multiview=True).cuda()
